...
O conjunto originalente tem xxxxx observações e xxxxxx variáveis.

Após as analises a base tem 1627945 observações e 32 variáveis.
...
```{r fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.width = 15}
...
b1= base |> 
  ...
  geom_boxplot() +
  labs(y= "Número de pessoas residentes na casa", x=" ",
       title="Perfil dos candidatos segundo número de pessoas residentes na casa",
       subtitle="Dados referentes aos candidatos do ENEM 2016 aprovados em pedagogia na UFF em 2017")+ theme_classic()+ theme(legend.position = "none", plot.title = element_text(hjust = 0.2, size=14, face="bold"), text = element_text(size=15), plot.subtitle = element_text(hjust = 0.2, size=12))
...
```{r fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.width = 14}
# calculando a media e as diversas medidas para as barras
media_erros <- base |> 
  group_by(ESCOL_MAE) |> 
  summarize(n = n(),
         media = mean(MORA),
...
### Aprovação na pedagogia x Idade
```{r fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.width = 14}
# gráfico de Aprovação na pedagogia x idade
base = base |> 
  mutate(AP = ifelse(Situação_A1.1=="Aprovado"|
           Situação_A2.1=="Aprovado" |
           Situação_A6.1=="Aprovado"|
...
base |>
  ...
  labs(title = "Distribuição da idade por situação de aprovação em pedagogia na UFF em 2017",
       y = "Densidade",
       subtitle="Dados referentes aos candidatos do ENEM 2016 (população geral)") + theme_classic() +
  theme(legend.position = "top")  +
  theme(plot.title = element_text(hjust = 0.5, size=14, face="bold"),
